...
router.post('/api/calculate-emission', async (req, res) => {
  try {
    ...
    currItem.paramValList[currItem.paramNameList.length == 3 ? 1 : 0] = value;
...
router.post('/api/estimate', async (req, res) => {
  try {
    ...
    const unit = requestBody[0].unit == 0 ? 1 : requestBody[0].unit == 1 ? 7 : 30;
    const responseBody = [];

    for (let i = 1; i < requestBody.length; i++) {
      const key = requestBody[i].name;
      const val = parseInt(requestBody[i].value);
      // responseBody.push({ name: key, value: fetchedData.co2e });
    }

    const recommendations = generateRecommendation(responseBody, unit);

    await DataSchema.create({ data: responseBody, recommendations }).then(function(element){
        res.send(element);
    }).catch(next);

    res.status(200).json({ data: responseBody, recommendations });
  } catch (error) {
    console.error('Error processing request', error);
    res.status(500).json({ error: 'Internal server error' });
  }
